run_subtype_interpretation_qc <- function(cfg) {
  ...
  has_min5 <- min_size >= 5
  recommendation <- if (has_min5 && nrow(subtype_counts) >= 2) "stable" else if (min_size >= 3) "exploratory" else "unstable"
  qc <- data.frame(
  ...
  if (!has_min5) message("WARNING: IgAN subtype QC: minimum subtype size is ", min_size, "; label this as an exploratory small-cluster subtype.")
  ...
  # Write small-cluster outputs BEFORE any plotting, so they cannot be skipped by graphics errors.
  ...
  if (length(small_clusters) > 0) {
    ...
    utils::write.csv(subtypes, file.path(tables_dir, "IgA_subtypes_without_small_cluster.csv"), row.names = FALSE)
  }

  # Robust clinical/pathology summary. GEO metadata often contains mostly constant technical fields.
  ...
  for (col in setdiff(colnames(merged), exclude_cols)) {
    ...
    if (sum(!is.na(values)) >= 2 && length(unique(values[!is.na(values)])) > 1) numeric_cols <- c(numeric_cols, col)
  }
  ...
  if (length(clinical_summary_list) > 0) {
    ...
    clinical_summary <- data.frame(note = "No informative numeric clinical or pathology features were detected in the available GSE104948 metadata.", stringsAsFactors = FALSE)
  }
  ...
  # Optional diagnostic placeholder PDFs. Use finite y limits to avoid plot.window errors.
  try({
    grDevices::pdf(file.path(figures_dir, "consensus_CDF_plot.pdf"), width = 6, height = 4)
    graphics::plot(2:4, c(0, 0.5, 1), type = "n", ylim = c(0, 1), xlab = "k", ylab = "Consensus CDF", main = "Consensus CDF diagnostic")
    graphics::text(3, 0.5, "Inspect ConsensusClusterPlus output PDFs\nfor full CDF diagnostics")
    grDevices::dev.off()
  }, silent = TRUE)

  try({
    grDevices::pdf(file.path(figures_dir, "consensus_delta_area_plot.pdf"), width = 6, height = 4)
    graphics::plot(2:4, c(0, 0.5, 1), type = "n", ylim = c(0, 1), xlab = "k", ylab = "Delta area", main = "Consensus delta area diagnostic")
